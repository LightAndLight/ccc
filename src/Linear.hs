...
usedIn :: String -> [String] -> Either Error ([String], ExprC)
usedIn name [] = Left $ UnusedVariable name
usedIn name (name' : ctx) =
  if name == name'
    then pure (ctx, IdC)
    else do
      (ctx', arrow) <- usedIn name ctx
      pure
        ( name' : ctx',
...
merge lctx (name : rctx) =
  if name `elem` lctx
    then Left $ DuplicateUse name
    else do
    ...
    go names expr =
      case expr of
        ...
        Lam name body -> do
          ...
          (ctx', swizzle) <- usedIn name ctx
          pure (ctx', AbsC $ ComposeC body' swizzle)
        App f x -> do
          (lctx, f') <- go names f
          (rctx, x') <- go names x
          (ctx, split) <- merge lctx rctx
        ...
        LetPair lname rname value body -> do
          ...
          (bodyCtx', reassocr) <- usedIn rname bodyCtx
          (bodyCtx'', reassocl) <- usedIn lname bodyCtx'
